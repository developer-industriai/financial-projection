...
class SaaSFinancialModel:
    ...
    def calculate_uk_corporation_tax(self, profit, year):
        ...
        if year < 2023:
            ...
            if profit <= 50000:
                ...
                marginal_rate = (profit - 50000) / (250000 - 50000) * (
                    0.25 - 0.19
                ) + 0.19
    ...
    # @staticmethod
    def run_monte_carlo(
        self, initial_investment, revenues, costs, discount_rate, iterations=1000
    ):
        npv_results = []
        irr_results = []

        for _ in range(iterations):
            revenue_mult = norm.rvs(
                1, 0.1
            )  # Revenue multiplier with 10% standard deviation
            ...
            taxes = [
                self.calculate_uk_corporation_tax(ebitda, year)
                for year, ebitda in enumerate(adjusted_ebitda, start=2023)
            ]
    ...
    def calculate_hr_resources(self, customer_base, years):
        employees_json_path = "data/employees-v02.json"
        self.roles = pd.read_json(employees_json_path)
        ...
        for year in range(1, years + 1):
            ...
            for role in self.roles:
                if role["start_year"] <= year <= role["end_year"]:
                    ...
                    if role["title"] == "Customer Support Specialist":
                        count = max(1, int(customer_count / self.ONE_CS_PER_CUSTOMERS))

                    elif role["title"] == "Sales Representative":
                        count = max(
                            1, int(customer_count / self.ONE_SALES_REP_PER_CUSTOMERS)
                        )

                    elif role["title"] == "Software Engineer":
                        count = max(
                            1, int(customer_count / self.ONE_ENGINEER_PER_CUSTOMER)
                        )

                    else:
                        base_count = max(1, int(customer_count / 200))
                        count = base_count + (
                            base_count // self.ONE_MANAGER_PER_EMPLOYEES
                        )
                    ...
                    for i in range(count):
                        ...
                        bonus = salary * role["bonus"]
                        total = (salary + labor_charges + benefits + bonus) * allocation
    ...
    def validate_cost_structure(
        sales_marketing, customer_success, admin, operations, engineering
    ):
    ...
    def calculate_services_costs(years, employee_counts, customer_counts):
        costs = []
        for year, employees, customers in zip(
            range(1, years + 1), employee_counts, customer_counts
        ):
            accounting_cost = 2000 * (
                1 + 0.1 * (year - 1)
            )  # Base 2000/year, 10% increase each year
            legal_cost = 10000 * (
                1 + 0.05 * (year - 1)
            )  # Base 10000/year, 5% increase each year
            rent_cost = (
                200 * 12 * employees * 0.5
            )  # £200/person/month * 50% ocupancy on hybrid model
            office_materials = 20 * employees  # £20/person/year
            software_licenses = 50 * employees  # £1000/person/year for various software
            marketing_cost = (
                5000 * (1 + 0.2 * (year - 1)) + 50 * customers
            )  # Base 5000/year, 20% increase each year, plus £50 per customer
            misc_cost = 2000 * (
                1 + 0.05 * (year - 1)
            )  # Base 10000/year, 5% increase each year
    ...
    def render_sidebar_parameters(self):
        ...
        self.market_size = st.sidebar.number_input(
            "Market Size  (number of customers)",
            min_value=0,
            value=self.TOTAL_NUMBER_OF_SCHOOLS,
            step=10000,
        )
        self.initial_investment = st.sidebar.number_input(
            "Initial Investment (£)", min_value=0, value=500000, step=10000
        )
        self.initial_customers = st.sidebar.number_input(
            "Initial number of customers", min_value=1, value=10
        )
        ...
        self.max_monthly_growth_rate = (
            (self.market_size / self.initial_customers) ** (1 / self.months) - 1
        ) * 100
        self.monthly_growth_rate = (
            st.sidebar.slider(
                "Monthly growth rate (%)",
                min_value=0.0,
                max_value=self.max_monthly_growth_rate,
                value=9.2,
            )
            / 100
        )
        self.churn_rate = (
            st.sidebar.slider("Monthly churn rate (%)", 0.0, 5.0, 1.0) / 100
        )
        self.annual_subscription_price = st.sidebar.number_input(
            "Annual subscription price (£)", min_value=0, value=10000
        )
        self.cogs_per_customer = st.sidebar.number_input(
            "Cost of Goods Sold (COGS) per customer (£)", min_value=0, value=500
        )
        self.discount_rate = (
            st.sidebar.slider("Discount rate (%)", 0.0, 20.0, 10.0) / 100
        )
    ...
    def calculate_projections(self):
        ...
        for _ in range(1, self.months):
            ...
            self.customer_base.append(
                self.customer_base[-1] + new_customers - churned_customers
            )

        self.monthly_revenues = [
            c * (self.annual_subscription_price / 12) for c in self.customer_base
        ]
        self.annual_revenues = [
            sum(self.monthly_revenues[i : i + 12]) for i in range(0, self.months, 12)
        ]
    ...
    def render_hr_resources(self):
        ...
        self.hr_df = self.calculate_hr_resources(
            [int(self.customer_base[i]) for i in range(11, self.months, 12)], 5
        )
        ...
        self.hr_costs_per_year = (
            self.edited_hr_df.groupby("Start Year")["Total"].sum().reset_index()
        )
        self.hr_costs_per_year.columns = ["Year", "Total HR Costs"]

        st.subheader("Total HR Costs per Year")
        st.dataframe(
            self.hr_costs_per_year.style.format({"Total HR Costs": "£{:,.0f}"})
        )

        st.subheader("Total HR Costs per Year, per Role")

        # Prepare data for the chart
        self.hr_costs_by_role = (
            self.edited_hr_df.groupby(["Start Year", "Role"])["Total"].sum().unstack()
        )
        ...
        self.hr_costs_percentage = self.hr_costs_by_role.apply(
        ...
        # Prepare data for the chart
        self.hr_costs_by_role = (
            self.edited_hr_df.groupby(["Start Year", "Role"])["Total"].count().unstack()
        )
        ...
        self.hr_costs_by_role["Customers"] = [
            int(self.customer_base[i]) for i in range(11, self.months, 12)
        ]
        # reorder columns
        self.hr_costs_by_role = self.hr_costs_by_role[
            ["Customers"]
            + [col for col in self.hr_costs_by_role.columns if col != "Customers"]
        ]
        # Add a last column with the total headcount for each year, excluding the 'Customers' column
        self.hr_costs_by_role["Total Headcount"] = (
            self.hr_costs_by_role.sum(axis=1) - self.hr_costs_by_role["Customers"]
        )
        ...
        # Prepare data for the chart
        self.hr_costs_by_area = (
            self.edited_hr_df.groupby(["Start Year", "Area"])["Total"].sum().unstack()
        )
        ...
        self.hr_costs_percentage_area = self.hr_costs_by_area.apply(
            lambda x: x / x.sum() * 100, axis=1
        )
        ...
        self.customer_counts = [
            self.customer_base[i] for i in range(11, self.months, 12)
        ]
        self.services_costs_df = self.calculate_services_costs(
            5, self.employee_counts, self.customer_counts
        )

    def render_services_costs(self):
        st.subheader("Services Costs")
        st.dataframe(
            self.services_costs_df.style.format(
                {
                    col: "£{:,.0f}"
                    for col in self.services_costs_df.columns
                    if col != "Year"
                }
            )
        )
    ...
    def render_5years_projection(self):
        ...
        self.df = pd.DataFrame(
            {
                ...
                "COGS": [
                    self.cogs[i] for i in range(11, self.months, 12)
                ],  # [self.customer_base[i] * self.cogs_per_customer for i in range(11, self.months, 12)],
        ...
        self.df["Total Costs"] = (
            self.df["COGS"] + self.df["HR Costs"] + self.df["Additional Costs"]
        )
        # Calculate EBITDA, Tax, and Net Profit
        self.df["EBITDA"] = self.df["Revenue"] - self.df["Total Costs"]
        self.df["Tax"] = self.df["EBITDA"].apply(
            lambda x: self.calculate_uk_corporation_tax(x, 2023)
        )
        ...
        self.customer_cagr = self.calculate_cagr(
            self.df["Customers"].iloc[0], self.df["Customers"].iloc[-1], 5
        )
        self.revenue_cagr = self.calculate_cagr(
            self.df["Revenue"].iloc[0], self.df["Revenue"].iloc[-1], 5
        )
        ...
        self.growth_rates = pd.DataFrame(
            {
                "Metric": [
                    "Monthly Growth Rate",
                    "Customer CAGR (5 years)",
                    "Revenue CAGR (5 years)",
                ],
                "Rate": [
                    self.monthly_growth,
                    self.customer_cagr * 100,
                    self.revenue_cagr * 100,
                ],
            }
        )

        st.subheader("Growth Rates Summary")
        st.dataframe(self.growth_rates.style.format({"Rate": "{:.2f}%"}))

    def render_financial_indicators(self):
        # Monte Carlo simulation
        st.subheader("Monte Carlo Simulation")
        self.npv_results, self.irr_results = self.run_monte_carlo(
            self.initial_investment,
            np.array(self.annual_revenues),
            np.array(self.df["Total Costs"]),
            self.discount_rate,
        )
        ...
        ax2 = ax1.twinx()
        ax2.plot(
            range(1, 61), self.monthly_revenues, color="green", label="Monthly Revenue"
        )
    ...
    def render_profit_loss(self):
        ...
        for i, v in enumerate(profit_loss):
            ax.text(
                years[i], v, f"£{v:,.0f}", ha="center", va="bottom" if v > 0 else "top"
            )
    ...
    def render_anual_budget(self):
        ...
        ax.bar(
            ...
            bottom=[i + j for i, j in zip(self.sm_costs, self.engineering_costs)],
            label="Operations",
            alpha=0.8,
        )
        ax.bar(
            ...
            bottom=[
                i + j + k
                for i, j, k in zip(
                    self.sm_costs, self.engineering_costs, self.operations_costs
                )
            ],
        ...
        for i, year in enumerate(years):
            total = (
                self.sm_costs[i]
                + self.engineering_costs[i]
                + self.operations_costs[i]
                + self.admin_costs[i]
            )
        ...
        st.dataframe(
            budget_breakdown.style.format(
                {col: "£{:,.0f}" for col in budget_breakdown.columns if col != "Year"}
            )
        )
    ...
    def render_5year_PandL_statement(self):
        ...
        for i in range(5):
            year_data = [
                ...
                -round(self.df["Customers"].iloc[i], 0)
                * self.cogs_per_customer,  # COGS
                0,  # Gross Profit
                -(
                    round(self.services_costs_df["Total Services Costs"].iloc[i], 2)
        ...
        fig, ax = plt.subplots(figsize=(12, 6))
        pl_df.set_index("Category").drop(columns="Total").loc[
            ["Revenue (Sales)"]
        ...
        st.markdown(
            "**Note:** This P&L statement is based on the projections and calculations from the financial model. All figures are in British Pounds (£)."
        )

    def render_3year_PandL_statement(self):
        st.subheader("3-Year Profit and Loss (P&L) Statement")
        ...
        for period in periods:
            data[period] = [0] * len(data["Category"])

        # Mapping from self.df columns to data categories
        df_to_category_mapping = {
            "Revenue": "Revenue (Sales)",
            "COGS": "Cost of Goods Sold (COGS)",
            "HR Costs": "Salaries, Benefits & Wages",
            "Additional Costs": "Total Expenses",
            "EBITDA": "EBITDA",
            "Tax": "Income Taxes",
            "Net Profit": "Net Earnings",
        }

        # Fill the data dictionary with self.df values based on the mapping
        for i, period in enumerate(periods):
            for df_col, category in df_to_category_mapping.items():
                if df_col in self.df.columns:
                    value = self.df[df_col].iloc[i]
                    if (
                        category != "Revenue (Sales)"
                        and category != "EBITDA"
                        and category != "Net Earnings"
                    ):
                        value = -value  # Make costs and expenses negative
                    data[period][data["Category"].index(category)] = round(value, 2)

            # Number of Customers
            data[period][data["Category"].index("Number of Customers")] = int(
                self.df["Customers"].iloc[i]
            )

            # Calculations for derived fields
            data[period][data["Category"].index("Gross Profit")] = (
                data[period][data["Category"].index("Revenue (Sales)")]
                + data[period][data["Category"].index("Cost of Goods Sold (COGS)")]
            )
            data[period][data["Category"].index("EBIT")] = (
                data[period][data["Category"].index("EBITDA")]
                + data[period][data["Category"].index("Depreciation & Amortisation")]
            )

        df_pl = pd.DataFrame(data)
        df_pl["Total"] = df_pl[periods].sum(axis=1, numeric_only=True)

        # Formatting
        formatted_df_pl = df_pl.style.format(
            {"Number of Customers": "{:,.0f}"}
            | {period: "£{:.0f}" for period in periods}
        )

        st.dataframe(formatted_df_pl)

        # Plot the P&L statement
        fig, ax = plt.subplots(figsize=(12, 6))
        df_pl.set_index("Category").drop(columns="Total").loc[
            ["Revenue (Sales)", "Number of Customers"]
        ].T.plot(kind="bar", ax=ax)
        ax.set_ylabel("Amount (£)")
        ax.set_title("Revenue and Customer Numbers Over 3 Years")
        plt.xticks(rotation=0)
        plt.tight_layout()
        st.pyplot(fig)

        st.markdown("**Note:** All figures are in British Pounds (£).")
        # st.subheader("3-Year Profit and Loss (P&L) Statement")
        # periods = ["Year 1", "Year 2", "Year 3"]
        ...
        #     for df_col, category in df_to_category_mapping.items():
        #         if df_col in self.df.columns:
        #             data[period][data["Category"].index(category)] = round(
        #                 self.df[df_col].iloc[i], 2
        #             )
        ...
        # format_dict = {period: "£{:.0f}".format for period in periods}
        # st.dataframe(df_pl.style.format(format_dict))
...
def main():
    ...
    model.render_3year_PandL_statement()
